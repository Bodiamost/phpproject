<?php 
function uploadImage($target_dir,&$target_name)
{
	$target_name = $target_name.'.'.pathinfo(basename( $_FILES["place_image"]["name"]),PATHINFO_EXTENSION);
	...
	// Check file size
	if ($_FILES["place_image"]["size"] > 500000) {
	...
	// if everything is ok, try to upload file
	    ...
	    if (move_uploaded_file($_FILES["place_image"]["tmp_name"], $target_file)) {
	        $message = $message. "The file ". basename( $_FILES["place_image"]["name"]). " has been uploaded.";
	    } else {
